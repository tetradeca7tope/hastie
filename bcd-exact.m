function [alphas, stats] = bcd(y, K_all, lambda_1, lambda_2, lambda_3)
...
% http://www.optimization-online.org/DB_FILE/2010/11/2806.pdf
    ...
    obj_history = objective(alphas,y,K_all,lambda_1,lambda_2,lambda_3)
    time_history = 0;
    tic;
    ...
    for iter=1:max_iters
        for g=randperm(m)
            ...
            if norm(K_g_y_minus_other,2) > lambda_3
                ...
                if iter==1
                    ...
                    [yy,~] = pcg(Lhs,K_g_y_minus_other,1e-2,[], ...
                        spdiags(diag(Lhs),0,n,n),spdiags(ones(n,1),0,n,n),yy_all(:,g));
                end
                yy_all(:,g) = yy;
                newalpha = Delta*yy;
            end
        ...
        obj = objective(alphas,y,K_all,lambda_1,lambda_2,lambda_3)
