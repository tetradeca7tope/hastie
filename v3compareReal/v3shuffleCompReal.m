...
allResults = zeros(numExperiments, numRegAlgos);
allTimes = zeros(numExperiments, numRegAlgos);
...
for expIter= 1:numExperiments
  ...
  fprintf('Experiment #%d: Dataset: %s (n, D) = (%d, %d)\n==========================\n', ...
            expIter, dataset, nTr, numDims);
  % Now run each method
  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  for i = 1:numRegAlgos
    ...
    allTimes(expIter, i) = cputime - startTime;
    predError = norm(YPred-Yte).^2/nTe;
    allResults(expIter, i) = predError;
    fprintf('Method: %s, err: %.5f\n\n', regAlgos{i}{1}, predError);
  end

end

results = mean(allResults)';
times = mean(allTimes)';
stdErrs = std(allResults)'/sqrt(numExperiments);
